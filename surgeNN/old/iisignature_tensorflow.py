...
def _sigGrad(op, grad):
    return tf.py_function(_sigGradImp,[grad]+list(op.inputs),
...
class _logSigGrad:
    ...
    def __call__(self,op,grad):
        ...
        return tf.py_function(fn,[grad]+list(op.inputs),
                      [tf.float32], name="LogSigGrad")

def _sigScaleGrad(op, grad):
    return tf.py_function(_sigScaleGradImp,[grad]+list(op.inputs),
                      [tf.float32]*3, name="SigScaleGrad")

def _sigJoinGrad(op, grad):
    return tf.py_function(_sigJoinGradImp,[grad]+list(op.inputs),
                      [tf.float32]*3, name="SigJoinGrad")

def _sigJoinGradFixed(op, grad):
    return tf.py_function(_sigJoinGradFixedImp,[grad]+list(op.inputs),
...
def Sig(x, m):
    ...
    g=tf.compat.v1.get_default_graph()#tf.get_default_graph()
    with g.gradient_override_map({"PyFunc":rnd_name}):
        return tf.py_function(_sigImp, [x,m], tf.float32, name="Sig") 
...
def LogSig(x, s, method=""):
    ...
    g=tf.compat.v1.get_default_graph()#tf.get_default_graph()
    with g.gradient_override_map({"PyFunc":rnd_name}):
        return tf.py_function(_logSigImp(s,method), [x], tf.float64, name="LogSig") 
...
def SigScale(x, y, m):
    ...
    g=tf.compat.v1.get_default_graph()#tf.get_default_graph()
    with g.gradient_override_map({"PyFunc":rnd_name}):
        return tf.py_function(_sigScaleImp, [x,y,m], tf.float32, name="SigScale") 
...
def SigJoin(x,y,m,fixedLast=None):
    ...
    if fixedLast is None:
        ...
        g=tf.compat.v1.get_default_graph()#tf.get_default_graph()
        with g.gradient_override_map({"PyFunc":rnd_name}):
            return tf.py_function(_sigJoinImp, [x,y,m], tf.float32, name="SigJoin")
    else:
        tf.RegisterGradient(rnd_name)(_sigJoinGradFixed)
        g=tf.compat.v1.get_default_graph()#tf.get_default_graph()
        with g.gradient_override_map({"PyFunc":rnd_name}):
            return tf.py_function(_sigJoinFixedImp, [x,y,m,fixedLast], tf.float32, name="SigJoin")
